...
export async function sendOrderConfirmationEmail({
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log(`[Dev/Build Mode] Skipping Resend order confirmation email for ${to}`)
    return
  }
...
export async function sendShippingUpdateEmail({
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log(`[Dev/Build Mode] Skipping Resend shipping email for ${to}`)
    return
  }
...
export async function sendPasswordResetEmail({
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log(`[Dev/Build Mode] Skipping Resend reset password email for ${to}`)
    return
  }
